...
and o.catalog=s.shopcoins ".($checking?"":"and s.`check`='1'")." and o.status=0;";*/
	...
	public function getDetails($user_id){
        ...
        if(!$orderdetails = $this->cache->load("orderdetails_".$clientdiscount.'_'.$this->getIdentity().'_'.$user_id)){	     
            $sql = "select o.*, o.amount as oamount, if
    				...
    				)as price, g.name as gname, c.materialtype, c.year, c.metal_id, c.group,c.name,c.number,c.image_small,c.image_big,c.width,c.height,c.condition_id,c.accessoryProducer,c.accessoryColors,c.accessorySize,c.weight from `orderdetails` as o, 
    shopcoins as c, `group` as g 
    where o.order='".$this->getIdentity()."' and o.catalog = c.shopcoins and (c.`check` in(1,4,5,6,7) ".($user_id==811?"or c.`check`>3":"").") and g.`group`=c.`group`  and o.status=0 and o.order>0
	...
	public function  getOrderDetails(){
	 ...
	 //такой же как в админке
	 public function PostSumForHtmluser($postindex,$clientdiscount,$vip_discount,$shopcoinsorder=0,$shopcoinsorders=array()){
    	...
    	$select = $this->db->select()
		               ->from($this->table,array('count(catalog)'))
		               ->where($this->table.".order in (".implode(",",$shopcoinsorder).")")		               
		               ->where('status=0'); 
		               
    	$bascetamount = $this->db->fetchOne($select); 
    	...
    	$select = $this->db->select()
			...
			->where($this->table.".order in (".implode(",",$shopcoinsorder).")")	
			->where('shopcoins.materialtype<>1 and shopcoins.materialtype<>2 and shopcoins.check=1 and orderdetails.status=0');
		$postcounter = $this->db->fetchOne($select);		
         
        $discountcoupon = 0;
    	$arraycoupcode = array();

		if($vip_discount){
			//var_dump(($bascetsum-$amountbascetsum-$vipcoinssum),$bascetsum,$amountbascetsum,$vipcoinssum);
			$discountcoupon = floor(($bascetsum-$amountbascetsum-$vipcoinssum)*$vip_discount/100);
			$typec = 2;
			$arraycoupcode[] = "VIP";
		} else {
			$sql = "select coupon.* from ordercoupon, coupon where ".(sizeof($shopcoinsorder)>1?"ordercoupon.order in (".implode(",",$shopcoinsorder).")":"ordercoupon.order='".$shopcoinsorder[0]."'")." and ordercoupon.order>0 and ordercoupon.`check`=1 and coupon.type=1 and coupon.coupon=ordercoupon.coupon group by coupon.coupon order by coupon.type desc, coupon.dateinsert desc;";
    	...
    	$select = $this->db->select()
			...
			->where("`order` in (".implode(",",$shopcoinsorder).")");
		 $suminsurance = $this->db->fetchOne($select);
    	...
    	//echo "<!--".var_export($shopcoinsorder,true)."  end-->";
    	return array('bascetamount'=>$bascetamount,'PostZoneNumber'=>$PostZoneNumber,'PostZonePrice'=>$PostZonePrice,'PostAllPrice'=>$PostAllPrice,'suminsurance'=>$suminsurance,'amountbascetsum'=>$amountbascetsum,'bascetsum'=>$bascetsum,'bascetpostweight'=>$bascetpostweight,'PostAllPrice'=>$PostAllPrice, 'arraycoupcode'=>$arraycoupcode,'discountcoupon'=>$discountcoupon);
