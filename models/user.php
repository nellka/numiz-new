...
{		
    ...
    public function updateVbullUser($data=array(),$login=''){
    	if(!$this->user_id||!$login) return false;
    	$this->db->update('vbull_user',$data,"username='".$login."'"); 
    	return ; 
    }
    
    public function updateForumPwd($pwd,$login=''){
	 	if(!$pwd||!$login) return false;
	 	$salt = $this->fetch_user_salt();  
	 	$data = array('password' => $this->hash_password($pwd, $salt), 
	 				  'salt'     => $salt);
	  	$this->updateVbullUser($data,$login);
	 	return ;
	 }
	...
	//добавляем нового пользователя и подписываем на новости если надо
	 ...
	 public function is_user_has_premissons(){
	 	 if($this->user_id == 357632) return false;
...
// а надо вот так
		...
		setcookie($cookie_prefix .'sessionhash', $sessionhash, $expire, '/');
		setcookie($cookie_prefix .'lastvisit', $now, $expire, $cookie_path, '/');
		setcookie($cookie_prefix .'lastactivity', $now, $expire, $cookie_path, '/');
		setcookie($cookie_prefix .'userid', $vbuser['userid'], $expire, $cookie_path, '/');
		//setcookie($cookie_prefix .'password', md5($vbuser['password'] . variable_get('drupalvb_license', '')), $expire, $cookie_path, $vb_cookie_domain);
		setcookie($cookie_prefix .'password', md5($vbuser['password']), $expire, '/');
		return TRUE;
	}
	...
	*/
		...
		setcookie("bbsessionhash", "0", time()-3600, "/");
		setcookie("bbpassword", "", time()-3600, "/");	
	...
	public function getMyMessage(){
		if(!$this->user_id) return false;
		$sql = "select cm.collectors_message, FROM_UNIXTIME(cm.date, \"%d.%m.%Y\") as date1, 
		u.fio, cm.topic, cm.view from collectors_message as cm left join user as u
		on cm.user_from = u.user where cm.user_to='".$this->user_id."'
		order by cm.date desc, cm.view limit 10;";
	...
	public function Message_show($id){
		...
		$data = array('view' => 1);
		$this->db->update('collectors_message',$data,"collectors_message = '".$id."' and user_to= ".$this->user_id);    	
		//оно просмотрено
				
		$sql = "select cm.collectors_message, FROM_UNIXTIME(cm.date, \"%d.%m.%Y\") as date1,user_from,user_to,
		u.fio, cm.topic, cm.details from collectors_message as cm left join user as u
		on cm.user_from = u.user where cm.user_to='".$this->user_id."' and 
		cm.collectors_message = '".$id."'
	...
	public function Message_delete ($id){
		if(!$this->user_id||!$id) return false;
		$this->db->delete('collectors_message',"user_to='".$this->user_id."' and collectors_message='".$id."'");
		return;
	}
