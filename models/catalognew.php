...
{	
	...
	public function countAllByParams($WhereParams=array()){	
		$select = $this->db->select();
		if($this->isDetectors()){
			$select->from(array('s'=>'catalogmetaldetector'),array('count(*)'));
		} else {
       ...
       */
		if (isset($WhereParams['year'])) {
			...
			foreach ($WhereParams['year'] as $year_int){
				if($year_int[0]>0&&$year_int[1]>0&&$year_int[1]<date('Y',time())){
					$where_year[] = "(`yearstart` >={$year_int[0]} and `yearend` <={$year_int[1]})";
				}elseif($year_int[0]==0&&$year_int[1]>0&&$year_int[1]<date('Y',time())){
					$where_year[] = "(`yearstart` >0 and `yearend` <={$year_int[1]})";
				} elseif ($year_int[0]>0){
					$where_year[] = "(`yearstart` >={$year_int[0]})";
				} else {
					$where_year[] = "(`yearend` <={$year_int[1]})";
				}
			}
		...
		if (isset($WhereParams['year_p'])) {
			$select->where("s.yearstart in (".implode(",",$WhereParams['year_p']).")");
		}
        ...
        if(($this->materialtype==1||$this->materialtype==8)&&$WhereParams['group']){
	...
	$limit;";
	    ...
	    */	    
		...
		if($this->isDetectors()){
			$select->from(array('s'=>'catalogmetaldetector'),ARRAY('*','catalog'=>'catalogmetaldetector')); 
		} else {
	...
	public function getGroups($WhereParams=array()){	
		...
		if($this->isDetectors()){
			$select->from(array('s'=>'catalogmetaldetector'),array('distinct(s.group)')); 
		} else {
	...
	protected function byParams($select,$WhereParams=array()){	
		...
		if(isset($WhereParams['catalognewmycatalog_usermycatalog'])||isset($WhereParams['catalognewmycatalog_usermycatalogchange'])) {     
            ...
            if($WhereParams['catalognewmycatalog_usermycatalog']){
		      $select->where("catalognewmycatalog.user=?",$WhereParams['catalognewmycatalog_usermycatalog']); 	
            }	
            
            if($WhereParams['catalognewmycatalog_usermycatalogchange']){
	...
	public function getMetalls($all=false,$groups=array(),$nominals=array()){	  	   
		...
		if($this->isDetectors()){
			$select->from(array('s'=>'catalogmetaldetector'),array('distinct(s.metal)')); 
		} else {
		...
		$select->join(array('metal'),'s.metal=metal.metal',array('name'=>'metal.name')); 
		           
		$select = $this->byParams($select,$WhereParams);    	   
    		
	    $select->where('s.metal>0 ')             
	            ->order('metal.name asc');	                  
