...
class stats {
	...
	public function getViewdNovelty($count=10){
		$data = array();
		
	  	$select  =  $this->db->select()
                  ->from('shopcoins_view_novelty',array('shopcoins'))
                  ->where('cnt>?',$count)
                  ->where('user_id=?',$this->user_id)
                  ->where('session_id = ?',$this->user_id?0:$this->session_int) ;
	...
	public function saveCoinsNovelty($shopcoins=0){
	   if($shopcoins){
	    	$sql = "INSERT INTO shopcoins_view_novelty  (user_id,session_id, shopcoins,cnt) VALUES ('".$this->user_id."','".($this->user_id?0:$this->session_int)."', '$shopcoins',1) ON DUPLICATE KEY UPDATE cnt = cnt + 1;";
	    	$this->db->query($sql);       
