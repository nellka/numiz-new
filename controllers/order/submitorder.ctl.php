...
$logger_coins->log($_SERVER['REMOTE_ADDR']. ' '.$_SERVER['HTTP_USER_AGENT']. ' '.' viporder:'.$viporder.', start submit order by user:'.$tpl['user']['user_id'].', order:'.$shopcoinsorder,Zend_Log::INFO); 		
		...
		if (($sum <= 0.00 || !$sum || $FinalSum <=0.00 || !$FinalSum) && $discountcoupon<=0) {
			...
			foreach ($result as $rows) {
				...
				if($data_update&&$data_update['check']===0){						
					$logger_coins->log($_SERVER['REMOTE_ADDR']. ' '.$_SERVER['HTTP_USER_AGENT']. ' '.' viporder:'.$viporder.', bye '.$rows["catalog"].', user:'.$tpl['user']['user_id'].', order:'.$shopcoinsorder,Zend_Log::INFO); 		
...
$logger_coins->log($_SERVER['REMOTE_ADDR']. ' '.$_SERVER['HTTP_USER_AGENT']. ' '.' viporder:'.$viporder.', end submit order by user:'.$tpl['user']['user_id'].', order:'.$shopcoinsorder,Zend_Log::INFO); 	