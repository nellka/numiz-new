...
$page2 = request("page2");
$l_order = (int)((isset($_COOKIE['l_order'])&&$_COOKIE['l_order']>0)?trim($_COOKIE['l_order']):0);
...
if (!$tpl['user']['user_id']){
	...
	if ($action=="showorders")	{		
		...
		foreach ($tpl['orders'] as $rows) {				    	
			...
			//if (!$rows["SendPost"] && !$rows["ReceiptMoney"] && !$rows["SendPostBanderoleNumber"] && $rows["ParentOrder"]==0 && (($rows['payment'] !=1 && $rows['payment'] !=2 && ($rows['delivery']==4 || $rows['delivery']==6)) || (($rows['delivery']==10 || $rows['delivery']==2) && )))
			{
				...
				if ($rows["delivery"] == 4 || $rows["delivery"] == 6 || $rows['delivery']==10 || $rows['delivery']==2) {
					...
					if($user_data['vip_discoint']&&(!$user_data['vip_discoint_date_end']||$user_data['vip_discoint_date_end']>$rows['date'])){
						$vip_discoint = $user_data['vip_discoint'];			
					}
		
					$bastet_details = $orderdetails_class->PostSumForHtmluser($postindex, $clientdiscount, $vip_discoint,$shopcoinsorders,$shopcoinsorders);
			...
			//	var_dump($resultsum);
				...
				$tpl['orders'][$i]['resultsum'] = $resultsum;
		...
		if($tpl['order']['user']!=$tpl['user']['user_id']){
	    	...
	    	if($user_data['vip_discoint']&&(!$user_data['vip_discoint_date_end']||$user_data['vip_discoint_date_end']>$tpl['order']['date'])){
				$vip_discoint = $user_data['vip_discoint'];
	
			}

			$bastet_details = $orderdetails_class->PostSumForHtmluser($postindex, $clientdiscount, $vip_discoint,array($order));
	        
	        $tpl['order']['bastet_details'] = $bastet_details;
	        
			$bascetsum = $bastet_details['bascetsum'];
			$bascetpostweight = $bastet_details['bascetpostweight'];	
			$PostAllPrice = $bastet_details['PostAllPrice'];	
			//получаем скидку по купону
	        $discountcoupon = (int)$tpl['order']['bastet_details']['discountcoupon']  ;	
			//монеты
			if ($type =="shopcoins"){	
