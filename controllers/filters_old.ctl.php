...
if(in_array($materialtype,array(1,7,8,6,4))){
	$tpl['filters']['metalls'] = $cache->load("metalls_$cache_prefix");
	if($tpl['filters']['metalls']===false) {	   
...
*/
if(in_array($materialtype,array(1,7,8,6,4))){
	$tpl['filters']['conditions'] = $cache->load("conditions_$cache_prefix");
	if($tpl['filters']['conditions']===false) {	   
...
if($materialtype&&$group_data){
	$tpl['filters']['series'] = $cache->load("series_$cache_prefix".implode("_",$group_data));
	
	if($tpl['filters']['series']===false) { 
