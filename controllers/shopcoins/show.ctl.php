...
if ($catalog){	
	...
	if ($rows_main) {
	 ...
	 if ($rows_main&&$rows_main['check'] == 0) {
		...
		$resultcicle = $shopcoins_class->getPopular(4, array('materialtype' => $materialtype, 'shopcoins.group' => $rows_main["group"]));
        
		$i=0;
		foreach ($resultcicle as $rows){        	
        	$row = $shopcoins_class->getItem($rows["shopcoins"],true);        	
        	$tpl['show']['resultcicle'][$i] =  array_merge($row,$rows);            	   
        	$i++;     	      	 
        }
