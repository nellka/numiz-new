...
if(!$search) {  
	$sql_all="SELECT count(s.shopcoins) FROM `shopcoins_search` AS `s`
 INNER JOIN `group` ON s.group=group.group
 LEFT JOIN `shopcoins_search_name` ON shopcoins_search_name.id=nominal_id
 LEFT JOIN `shopcoins_search_details` ON shopcoins=catalog WHERE (s.check=1) AND (metal_id='1'
and s.group in (561, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574))";
}
...
if(!$search) { 
    $sql = "SELECT `s`.*, `group`.`name` AS `gname`, `shopcoins_search_name`.*, `shopcoins_search_details`.* FROM `shopcoins_search` AS `s`
 INNER JOIN `group` ON s.group=group.group
 LEFT JOIN `shopcoins_search_name` ON shopcoins_search_name.id=nominal_id
 LEFT JOIN `shopcoins_search_details` ON shopcoins=catalog WHERE (s.check=1) AND (metal_id='1'
and s.group in (561, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574)) $orderby limit ".($tpl['pagenum']-1)*$tpl['onpage'].",".$tpl['onpage'];    
}
...
if (sizeof($tpl['shop']['MyShowArray'])==0){
	$tpl['shop']['errors'][] = "<br><p class=txt><strong><font color=red>�?звините, нет результатов, удовлетворяющих поиску. Попробуйте другие варианты.</font></strong><br><br>";
} else {	
