...
//desc - asc
//start - потом не забыть подключить - берем из кук
$tpl['orderby'] = request('orderby')?request('orderby'):"dateinsert";
$tpl['onpage'] = request('onpage')?request('onpage'):9;
...
$countpubs = $shopcoins_class->countallByParams($tpl['user']['user_id'],$nocheck,$materialtype,$WhereArray,$searchname,$yearsearch);
...
var_dump($OrderByArray,$tpl['orderby']);
...
elseif ($page == "recommendation" && $tpl['user']['user_id']) {
	...
	$limit;";
	echo $sql;*/
	$data = $shopcoins_class->getItemsByParams($tpl['user']['user_id'],$nocheck,$materialtype,$WhereArray,$searchname,$yearsearch,$tpl['pagenum'],$tpl['onpage'],$orderby);
