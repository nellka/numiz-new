...
if ($shopkey) {
	...
	$shopkey = str_replace("'","",$shopkey);	
		...
		WHERE ".$positive_amount."shopcoins.`group`=`group`.`group` AND (".($cookiesuser==811?(!$nocheck?"shopcoins.check=1 or (shopcoins.check>3 and shopcoins.check<20)":"(shopcoins.check>3 and shopcoins.check<20)"):"shopcoins.check=1").") and (shopcoins.materialtype = 1 or shopcoins.materialtypecross & pow(2,1)) and shopcoins.amountparent>0
...
elseif ($checkuser && $cookiesuser && ($num > 3) ) {
	...
	$sql = "select shopcoins.*, `group`.`name` as gname 
		...
		where ".$positive_amount."shopcoins.`group`=`group`.`group` AND (".($cookiesuser==811?(!$nocheck?"shopcoins.check=1 or (shopcoins.check>3 and shopcoins.check<20)":"(shopcoins.check>3 and shopcoins.check<20)"):"shopcoins.check=1").")  and (shopcoins.materialtype = 1 or shopcoins.materialtypecross & pow(2,1)) and shopcoins.amountparent>0
		and shopcoins.`shopcoins` = clientselectshopcoins.`shopcoins` 
		and clientselectshopcoins.`user` = '$cookiesuser' and clientselectshopcoins.`dateselect` >= '$dateselect' order by shopcoins.".$dateinsert_orderby." desc,shopcoins.price desc limit 0,$num;";
}
elseif ($page == "recommendation" && $cookiesuser) {
	...
	$sql = "select shopcoins.*, `group`.`name` as gname 
		from `shopcoins`, `group`, `clientselectshopcoins` 
		where ".$positive_amount."shopcoins.`group`=`group`.`group` AND (".($cookiesuser==811?(!$nocheck?"shopcoins.check=1 or (shopcoins.check>3 and shopcoins.check<20)":"(shopcoins.check>3 and shopcoins.check<20)"):"shopcoins.check=1").") and shopcoins.`shopcoins` = clientselectshopcoins.`shopcoins` 
		and clientselectshopcoins.`user` = '$cookiesuser' 
		...
		$positive_amount = '';
	if($materialtype == 2)
	$positive_amount = ' and shopcoins.amount > 0 ';
...
/*if ($REMOTE_ADDR == $myip)
{
	$sql = "select shopcoins.*, group.name as gname
	from shopcoins, `group` 
	where 1 and shopcoins.group=group.group 
	and shopcoins.check = 3 
	order by number 
	$limit;";
}*/
...
while ($rows = mysql_fetch_array($result_search))
{
...
if (sizeof($ArrayParent))
{
	...
	and (".($cookiesuser==811?(!$nocheck?"shopcoins.check=1 or (shopcoins.check>3 and shopcoins.check<20)":"(shopcoins.check>3 and shopcoins.check<20)"):"shopcoins.check=1").") order by parent;";
	if ($REMOTE_ADDR == $myip)
		echo $sql_search;
