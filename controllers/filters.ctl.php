...
if($materialtype==11){
	$tpl['filters']['prices'] = $shopcoins_class->getPrices($nominals,$groups);
	
	foreach ($tpl['filters']['prices'] as $value){
	    $childen_data_prices[] = array(
				'filter_id' => $value['price'],
				'name'      => $value['price']);   
	} 		
}
...
if($materialtype!=11){
	if(($nominals&&$groups)||($groups&&$materialtype==4)||$materialtype=='newcoins'){
...
if($childen_data_prices)  $filter_groups[] = $tpl['filter']['prices'] = array('name'=>'Цены','filter_group_id'=>'price','filter_group_id_full'=>'prices','filter'=>$childen_data_prices);
