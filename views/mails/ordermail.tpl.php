...
if ($tpl['submitorder']['discountcoupon']) {
    if($tpl['user']['vip_discoint']) {
	   ...
	   if($tpl['user']['vip_discoint_end']){
	   		$mail_text .= "Ваша скидка как VIP-клиента заканчивается <font color=red>".date("d.m.Y",$tpl['user']['vip_discoint_end'])."</font><br>";
	   }
   } else {
       $mail_text .= 'Сумма заказа: <font color="red">'.($tpl['submitorder']['sum']+$tpl['submitorder']['discountcoupon']).' руб.</font><br>';
       $mail_text .='Скидочный купон(ы):<font color="red">'.implode(", ",$tpl['coupons']).'</font><br>';
