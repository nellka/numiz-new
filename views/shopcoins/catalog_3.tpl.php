<?	echo $tpl['paginator']->printPager(); 
		...
		foreach ($MyShowArray as $key=>$rows)
		{
			...
			elseif (sizeof($ourcoinsorder) and in_array($rows["shopcoins"], $ourcoinsorder))
				...
				<a href='#coin".$rows["shopcoins"]."' onclick='javascript:AddAccessory_3_3(".$rows["shopcoins"].")'><div id=bascetshopcoins".$rows["shopcoins"]."><img src=".$in."images/corz7.gif border=0 alt='Уже есть в вашей корзине'></div></a>";
			else
			{
				...
				and !substr_count($HTTP_SERVER_VARS["HTTP_USER_AGENT"],"coona")
					...
					<a href='#coin".$rows["shopcoins"]."' onclick='javascript:AddAccessory_3(".$rows["shopcoins"].")'><div id=bascetshopcoins".$rows["shopcoins"]."><img src=".$in."images/corz1.gif border=0 alt='В корзину'></div></a>
					";
			...
			($rows['group']==1604?"Приблизительная цена":"Цена").": <strong><font color=red>".($rows["price"]==0?"бесплатно":round($rows["price"],2)." руб.")."</font></strong>".($rows["clientprice"]>0?"<br><a href=# onclick='javascript:alert(\"Цена со скидкой только для постоянных клиентов - сделавших не менее 3-х заказов за год под своим логином!\")' title='Цена со скидкой только для постоянных клиентов - сделавших не менее 3-х заказов за год под своим логином!' >Цена<b><sup><font color=blue>для постоянных клиентов</font></sup></b>: <strong><font color=red>".round($rows["clientprice"],2)." руб.</font></strong></a>":""));
		...
		//echo "<p class=txt><font color=blue> Под постоянным клиентом(покупателем) подразумевается покупатель сделавший не менее трех заказов в год. </font></p>";
		echo $page_print."<br>";