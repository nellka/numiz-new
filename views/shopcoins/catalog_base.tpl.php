...
<?   $i=1;
    foreach ($tpl['shop']['MyShowArray'] as $key=>$rows){	      
    	if(in_array($materialtype,array(7,4))){
    		echo "<div class='blockshop_spisok' id='item".$rows['shopcoins']."' itemscope itemtype=\"http://schema.org/Product\">";
    		include('items/item_nabor.tpl.php');
    		echo "</div>";
    	} else {
    		echo "<div class='blockshop' id='item".$rows['shopcoins']."' itemscope itemtype=\"http://schema.org/Product\">
...
<?
if($tpl['seo_data']&&($tpl['seo_data']['title']||$tpl['seo_data']['text'])){?>
<div class="seo">
...
if ($tpl['shop']['OtherMaterialData']) {	?>
	...
	<div class="triger-carusel">	
		...
		foreach ($tpl['shop']['OtherMaterialData'] as $rowsp){
		    ...
		    ?>			
			<li>
			<div class="coin_info" id='item<?=$rowsp['shopcoins']?>'  itemscope itemtype="http://schema.org/Product">
...
<script type="text/javascript">
 ...
 jQuery(document).ready(function() {  
 	<?if(!in_array($materialtype,array(4,7))) {?> 
 	   $('.product-grid').find(".qwk").each(function(i) {
...
function mCustomScrollbars(){	
	...
	$(".filter-groupyears_container_1").mCustomScrollbar({theme:"dark-thick"});	
	  ...
	  //  console.log(name);
	   ...
	   // console.log(name+','+$(".filter-group"+name+"_container_1").height());
		...
		if(cookiefull>0){
		   ...
		   /* if(name=='years'){
		    	var new_h =$(".filter-group"+name+"_container_1").height();
		    	$(".filter-group"+name+"_container_1").height(new_h/3);
		    }*/
