<div class="mainItemPhoto">
    ...
    <div id=Image_Big class="main_img_big  center">
    	<?=contentHelper::showImage($image,"Каталог монет - ".$rows_main["gname"]." - ".$rows_main["name"],array('alt'=>"Каталог монет - ".$rows["gname"]." ".$rows["name"],'folder'=>'catalognew'))?>	
    </div>	
...
<div  class="detailsItem">
   <div>
     ...
     if($tpl['catalognew']['show']['can_edit']){
		...
		echo "<a href=http://www.numizmatik.ru/new/?module=catalognew&task=addcoins&catalog=$catalog target=_blank><img src='".$cfg['site_dir']."catalognew/images/edit.gif' border=0 class='left'>&nbsp;&nbsp;Редактировать</a>";
		echo '</div>';
	}
	...
	if ($rows_main["agreement"]==0&&$tpl['catalognew']['show']['rows_agreement_catalog'])	{?>
		...
		<input type=Radio name='answercataloghistory[<?=$tpl['catalognew']['show']['rows_agreement_catalog']["cataloghistory"]?>]' <?=(${"answercataloghistory"}[$tpl['catalognew']['show']['rows_agreement_catalog']["cataloghistory"]]==2?"checked":"")?> value=2> Нет
		<input type=Radio name='answercataloghistory[<?=$tpl['catalognew']['show']['rows_agreement_catalog']["cataloghistory"]?>]' <?=(${"answercataloghistory"}[$tpl['catalognew']['show']['rows_agreement_catalog']["cataloghistory"]]==1?"checked":"")?> value=1> Да
	    <input type=submit name=submitcataloghistory value='Записать' class=formtxt>
		</form>
		</div>	
	<?} 
	...
	if ($tpl['catalognew']['show']['is_shopcoinssubbscribe']) {
		...
		if ($rows_main['materialtype'] == 3) {?>
			...
			<br><a href='#coin".$rows_main["catalog"]."' onclick=\"javascript:AcsessorySubscribe(".$rows_main["catalog"].");\" title='При появлении данного типа монеты ".$rows["gname"]." ".$rows_main["name"]." в магазине вам будет отправлено уведомление на email...'>Изменить заявку</a>
			";
		}		
		echo " [ <a href='#coin".$rows_main["catalog"]."' onclick=\"deleteSubscribeCatalog(".$rows_main["catalog"].");\">X</a> ]";		
	} else {
		if ($catalogshopcoins[$rows["catalog"]]!=1 && $rows_main['materialtype']!=3) {			
		...
		} elseif ($rows_main['materialtype'] == 3) {?>
            <div class="amount">
                <a href='#coin<?=$rows_main["catalog"]?>' onclick="addSubscribeCatalog(<?=$rows_main["catalog"]?>);" title='При появлении данного типа монеты <?=$rows_main["gname"]?> <?=$rows_main["name"]?> в магазине вам будет отправлено уведомление на email...'>Оставить заявку</a>
                <span class="down">-</span>
                <input type=text name=amountscribecatalog<?=$rows["catalog"]?> id=amountscribecatalog<?=$rows_main["catalog"]?> size=1 value='0'> 
                <span class="up">+</span>
            </div>	
		<?}				
	}
	...
	if ($rows_main['materialtype'] != 3) {
		...
		echo "<br><div id=mycatalog".$rows_main["catalog"].">";
