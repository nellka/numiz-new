<div>    
        <div class='left marg-10'><a href=about.php title='Подробно о каталоге монет, банкнот'>О каталоге</a></div>
        <div class='left marg-10'> | </div>
        <div class='left marg-10'><a href=how.php title='Как добавить монету, банкноту в каталог'>Как добавить</a></div>
        <div class='left marg-10 '> | </div>
        <div class='left marg-10'><a href=faq.php title='Вопросы по работе каталога монет, банкнот'>Вопрос-ответ</a></div>
</div>

<?
if($materialtype==1000){?>
   <br><br><div class="bordered center"><img src="<?=$cfg["site_dir"]?>images/banner_detektors.gif" border=0></div>
<?} elseif($tpl['catalognew']['user_id']) {?>
	...
	if ($tpl['user']['user_id'] == $tpl['catalognew']['user_id']) {
      ...
      <tr>
	   ...
	   <a href="<?=$cfg["site_dir"]?>catalognew/index.php?usermycatalog=<?=$tpl['user']['user_id']?>&materialtype=<?=$materialtype?>">
	    ...
	    <a href="<?=$cfg["site_dir"]?>catalognew/excelmycatalog.php" target=_blank><img src="<?=$cfg["site_dir"]?>catalognew/images/excel.gif" alt='Экспорт своей коллекции в Excel' border=0></a>
	    <br>
...
if($tpl['catalognew']['testaddcoins']){?>
    <a href="<?=$cfg["site_dir"]?>new/?module=catalognew&task=addcoins" target=_blank>Добавить монету</a><br><br>
<?}
...
<div class="filter-products-block">
    <?
    if($materialtype!==1000){
        include(DIR_TEMPLATE.'leftmenu/filters/catalognew/filters_new.tpl.php');
    }
    ?>
</div>
...
<? if($GroupName){?>
  ...
  <?if($tpl['is_Subscribe_for_group']){?> 
        <a href="<?=$cfg["site_dir"]?>new/?module=catalognew&usercatalogsubscribe=<?=$tpl['user']['user_id']?>&materialtype=<?=$materialtype?>">Вы подписаны на группу <?=$GroupName?></a>
  <?} elseif($tpl['user']['user_id']) {?>
      <a href=# onclick="ga('send', 'event', 'catalog', 'groubsubscribe');showWin('http://www.numizmatik.ru/new/?module=catalognew&task=subscribe&group=<?=$group?>&ajax=1','450');return false;">Подписаться на группу <?=$GroupName?></a>
 <?}?>
  </div>
<?}?>
</div>
<?
if(isset($tpl['GroupDescription'])&&$tpl['GroupDescription']) {?>
	<br class="clear:both">
	<br><br>
...
<?  
    foreach ($tpl['catalognew']['MyShowArray'] as $key=>$rows){	
    	IF($materialtype==1000){
    		...
    		include('items/item_detectors.tpl.php');
    		echo "</div>";
    	} elseif(in_array($materialtype,array(7,4))){
