...
<div id="myDivElement"><?=$details?></div>
...
<div class="wraper clearfix info_coins">
	<div style="display: inline-block;width:470px;">
		<div id=allreviews style="vertical-align:top;overflow:hidden;max-height:500px;">
			...
			<?} else {?>
			    <a href=# onclick="showWin('http://www.numizmatik.ru/new/?module=catalognew&task=subscribe&catalog=<?=$catalog?>&ajax=1','450');return false;">Подписаться на изменения о данной монете (заявки, обсуждение, цены)</a><br>
        	<?}?>
			<br>
			<div id=reviewsdiv >
			<? 
...
if ($tpl['catalognew']['show']['result_price'])	{?>
    <div class="wraper clearfix info_coins">
		...
		<? foreach ($tpl['catalognew']['show']['result_price'] as $rows_price) {
			...
			<td><a href="http://www.numizmatik.ru/shopcoins/<?=$rows_price["rehref"]?>" target=_blank>Посмотреть</b></td>
			</tr>
		<?}?>
		</table>
    </div>
...
if ($tpl['catalognew']['show']['offers']){?>
	...
	<form action=http://www.numizmatik.ru/new/?module=catalognew&task=show&catalog=<?=$catalog?> method=post>
	<table border=0 cellpadding=3 cellspacing=1 align=center width=100%>
...
<?} else {
	if ($CatalogHistoryAgreement)	{
		echo  "<table border=0 cellpadding=3 cellspacing=1 align=center width=100%>
		<tr class=tboard valign=top bgcolor=#ffcc66>
		<td>
		<b>
		<font color=red>".$CatalogHistoryAgreement."</font>
		</b></td>
		</tr>
		</table>";
	}
...
<div class="bordered">
	...
	if (in_array($rows_main["group"],$arraygroupbottom))
	    echo "<br><a href=".$in."shopcoins/albom_dlya_monet.html>Альбомы для монет для истинных ценителей</a><br>";
	?>
</div><br>
