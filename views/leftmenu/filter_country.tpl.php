<? 
$grParams = array();
$grParams['nocheck'] = $urlParams['nocheck'];
$grParams['mycoins'] = $urlParams['mycoins'];
$grParams['materialtype'] = $urlParams['materialtype'];   
?>
		...
		<ul class="filter_heading_ul">
			<div id="filter-groupgroup_container" class="filter-groupgroup_container_<?=(count($filter_group['filter'])>13)?1:0?>">
				...
				foreach ($groups_filter['filter'] as $filter) {
					...
					$grParams['group'] = array($filter['filter_id']=>$filter['name']);
					...
					<div class="checkbox">
					   ...
					   <a href="<?=urlBuild::makePrettyUrl($grParams)?>"> <?=$filter['name'];?></a>
						<?php } else { ?>
					...
					if(isset($filter['child'])){?>
						...
						<?foreach ($filter['child'] as $filter_child) {
							$grParams['group'] = array($filter_child['filter_id']=>$filter_child['name']);
							?>
							<div class="checkbox">
								<?php             
								if (is_array($groups)&&in_array($filter_child['filter_id'], $groups)) { ?>
								    ...
								    <a href="<?=urlBuild::makePrettyUrl($grParams)?>"> <?=$filter_child['name'];?></a>
								<?php } else { ?>
