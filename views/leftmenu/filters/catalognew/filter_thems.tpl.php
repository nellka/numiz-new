...
if (isset($tpl['filter']['theme'])) {
    ...
    $themeUrlParams['usershopcoinssubscribe'] = $urlParams['usershopcoinssubscribe'];
	$themeUrlParams['usercatalogsubscribe'] = $urlParams['usercatalogsubscribe'];
	$themeUrlParams['usermycatalog'] = $urlParams['usermycatalog'];
	$themeUrlParams['usermycatalogchange'] = $urlParams['usermycatalogchange'];
    ?>
				...
				foreach ($tpl['filter']['theme']['filter'] as $filter) {
					...
					<div class="checkbox">
                        <input type="checkbox" name="themes[]" value="<?=$filter['filter_id']?>" <?=(is_array($themes)&&in_array($filter['filter_id'], $themes))?"checked":""?>/>
					       <a href="<?=urlBuild::makePrettyUrl($themeUrlParams,"http://www.numizmatik.ru/catalognew")?>"> <?=$filter['name'];?></a>
					</div>
				<?}?>        
    		</div>
    	</ul> 	
