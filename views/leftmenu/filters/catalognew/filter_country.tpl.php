...
$search_href = '';
?>
<form id='search-params' method="POST" action="<?=$cfg['site_dir']?>new/?module=catalognew&materialtype=<?=$materialtype?>" class="left">
					...
					if(isset($filter['child'])){?>
						...
						<?foreach ($filter['child'] as $filter_child) {?>
							<div class="checkbox">
								...
								<?php } else { ?>
								    <input type="checkbox" name="groups[]" value="<?php echo $filter_child['filter_id']; ?>" onclick="addToFilterDetails('<?=$filter_child['filter_id']?>','<?=$filter_child['name'];?>')" />
								    <a href="<?=$r_url?>&group=<?=$filter_child['filter_id'].$search_href?>"> <?=$filter_child['name'];?></a>
								<?php } ?>
							</div>
							<?php     
