...
<?} else {
    include(DIR_TEMPLATE.'_mobile/catalognew/item/item.tpl.php');   
}?>
</div>

<? if(!$tpl['catalognew']['error']['no_coins']){ ?>

<div id="h-details" class="har" onclick="showInvis('allreviews');">
	Отзывы
	<div class="close-h-x"></div>
		<div id="allreviews" class="allreviews" style="display: none;">
			<a name="showreview"></a>
			<div id="reviewsdiv">				
				...
				<?}
					...
					echo "<div id=emptyreview>Комментариев пока что нет. Может, Вы будете первыми?</div>";
				}?>								
			</div>
		</div>
	</div>
...
<div id="h-details" class="har" onclick="showInvis('user-works');">
	Кто работал над монетой
	<div class="close-h-x"></div>
		<div id="user-works" class="user-works" style="display: none;">
