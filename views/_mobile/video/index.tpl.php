...
<table border=0 align=center class="table_cl">
	<tr><td class='<?=(!$theme?"table_active_block":"")?>'>
	...
	foreach ($arraythemevideo as $key=>$value) {
		...
		echo "<td class='".($theme==$key?"table_active_block":'')."'><a href=".$cfg['site_dir']."video.php?theme=$key title='".$value."'>$value</a></td>";
		$i++;
	}?>
	</tr>
</table>
...
function ShowVideo(numbervideo,typeplayer) {
	...
	if (typeplayer==1) {
		...
		str += '<embed src="http://www.youtube.com/v/'+numbervideo+'?autoplay=1&feature=player_embedded" type="application/x-shockwave-flash" allowfullscreen="true" allowScriptAccess="always" width="340" ></object>';
	}
	...
	<a class="h" href="#" onclick="return false;">Заявка на новое видео на сайте</a>
		...
		<div id="myLink" class="table videolink">
			...
			<p>
				...
				<input type=text name=videolink value='<?=$videolink?>' maxlength=200 size=27>	
			</p>
			<p>
				<label for="videolink">Что бы Вы желали увидеть еще в данном разделе:</label>
			
				<textarea name=text class=formtxt cols=40 rows=10><?=$text?></textarea>
			</p>
