...
<div class="center">
    ...
    <? if(!$tpl['user']['user_id']){?>
    	    <br><a class="button25" onclick="showWin('<?=$cfg['site_dir']?>user/login_order.php?ajax=1',500);return false;" href="#" style="width:150px" id='of-1'>Оформить заказ</a>
	<?} else {?>
        	<form action="<?=$cfg['site_dir']?>shopcoins?page=order&page2=1" method="post">
        	 <br> <input type=submit  class="button25" name=submit value='Оформить заказ' style="width:150px">
        	</form>
    	<?}?>
    <?} elseif($tpl['orderdetails']['ArrayShopcoinsInOrder']) { ?>
       <br> <input type="button"  class="button26" value='Оформить заказ'>
